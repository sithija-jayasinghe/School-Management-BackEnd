...
public class StudentMarkServiceImpl implements StudentMarkService {
    ...
    @Override
    public StudentMarkDTO createStudentMark(Long authenticatedUserId, StudentMarkDTO dto) {
        Exam exam = getActiveExam(dto.getExamId());
        validateExamAccess(authenticatedUserId, exam);
        ...
        applyRelations(authenticatedUserId, studentMark, dto, exam, student);
    ...
    @Override
    public StudentMarkDTO updateStudentMark(Long authenticatedUserId, Long id, StudentMarkDTO dto) {
        StudentMark studentMark = getStudentMark(id);
        validateExamAccess(authenticatedUserId, studentMark.getExam());
        ...
        applyRelations(authenticatedUserId, studentMark, resolved, exam, student);
        calculateResult(studentMark, exam);
    ...
    @Override
    public void deleteStudentMark(Long authenticatedUserId, Long id) {
        StudentMark studentMark = getStudentMark(id);
        validateExamAccess(authenticatedUserId, studentMark.getExam());
    ...
    private void applyRelations(Long authenticatedUserId, StudentMark studentMark, StudentMarkDTO dto, Exam exam, Student student) {
        studentMark.setExam(exam);
        studentMark.setStudent(student);
        studentMark.setEnteredBy(resolveEnteredByForCurrentUser(authenticatedUserId, dto.getEnteredByStaffId(), exam));
    }
    ...
    private void validateExamAccess(Long authenticatedUserId, Exam exam) {
        ...
        if (user.getRole() != Role.TEACHER) {
            throw new ResourceNotFoundException("Student marks access is not available for current user");
        }
        ...
        if (!assigned) {
            throw new ResourceNotFoundException("Exam not found in current teacher marks access");
        }
    }

    private Staff resolveEnteredByForCurrentUser(Long authenticatedUserId, Long fallbackStaffId, Exam exam) {
        if (authenticatedUserId != null) {
        ...
        Staff enteredBy = staffRepository.findByIdAndActiveTrue(fallbackStaffId)
                .orElseThrow(() -> new ResourceNotFoundException("Staff not found with id: " + fallbackStaffId));
